...
```{r antibodies, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, show_col_types=FALSE}

read_csv("antibodies.csv") %>% knitr::kable()
...
iii\) 5 mcl PageRuler Plus ladder  
iv\) run @ 225V for 30 -- 45 mins or 170V for 60 mins  
...
vii\) transfer on Mini Blot module @ 20V for 60 mins (for two gels: 350 mA dropping to 250 mA)    
...
### Adaptations for Tris-acetate gels:  

- use 4x loading buffer (NP0007)  
- use HiMark high-MW ladder (10 mcl gives quite faint bands)  
- run in Tris-acetate running buffer and add 500 mcl anti-oxidant (NP0005) to upper chamber  
- run at 150V for 60' (expect 40 - 55 mA dropping to 25 - 40 mA)  
- transfer in NuPage transfer buffer (1x) with 10% MeOH and 0.1% anti-oxidant (i.e. 1 ml in 1000 ml)...  
- ...having first pre-equilibrated gel in 2x transfer buffer without MeOH for 10 mins  
- transfer at 20V for 60' (expect 380 mA dropping to 130 mA)  
...
vi\) annotate with ECL pen  
vii\) ECL > LICOR Odyssesy
...
To work LICOR Odyssey M:  

- place blot protein side down and with higher MW towards user  
- at the end, select all images and `export to analyse` to export .zip file  
...
ii\) stain with Instant Blue for 5 mins   
iii\) destain in 50% MeOH (or 50% MeOH / 7% acetic acid?) for a few mins  
iv\) image on 630 Epi channel (or alternatively IR channels?)  
